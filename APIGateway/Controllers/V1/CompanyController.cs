...
{ 
   [ERPAuthorize]
    public class CompanyController : Controller
    {
        ...
        public CompanyController(DataContext dataContext, IWebHostEnvironment env, ILoggerService loggerService, UserManager<cor_useraccount> userManager,  IHttpContextAccessor httpContextAccessor, IMapper mapper, IMediator mediator, ICompanyRepository companyRepository)
        {
                    ...
                    if (item != null)
                    {
                        return BadRequest(new CompanyStructureDefinitionRegRespObj
                        {
                            Status = new APIResponseStatus
                            {
                                ...
                                Message = new APIResponseMessage
                                {
                                    FriendlyMessage = "Duplicate setup detected"
                                }
                            }
                        });
                    } 
        ...
        public async Task<IActionResult> GET_COMPANY_STRUCTURE(GetCompanyStructureQuery query)
        {
        ...
        {
            var query = new UploadCompanyStructureDefinitionCommand();
            var res = await _mediator.Send(query);
            if (!res.Status.IsSuccessful)
                return BadRequest(res);
            return Ok(res);
        ...
        {
            var query = new DownloadCompanyDefinitionQuery();
            return Ok(await _mediator.Send(query));
        }
