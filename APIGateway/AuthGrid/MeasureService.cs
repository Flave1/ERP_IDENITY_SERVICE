...
namespace APIGateway.AuthGrid
{
    public interface IMeasureService
    {
        ...
        Task<List<ScrewIdentifierGrid>> Return_auth_settings();
        Task<List<ScrewIdentifierGrid>> Return_auth_settings_for_central();
    }
    ...
    public class MeasureService : IMeasureService
    {
        ...
        public async Task<List<ScrewIdentifierGrid>> Return_auth_settings()
        {
            ...
            else
            {
                ...
                await _cacheService.CatcheResponseAsync(CacheKeys.AuthSettings, lockoutSetting);
            }
            return lockoutSetting;
        }

        public async Task<List<ScrewIdentifierGrid>> Return_auth_settings_for_central()
        {
            var lockoutSetting = new List<ScrewIdentifierGrid>();

            var cachedSetting = await _cacheService.GetCacheResponseAsync(CacheKeys.central_authSettings);

            if (!string.IsNullOrEmpty(cachedSetting))
                lockoutSetting = JsonConvert.DeserializeObject<List<ScrewIdentifierGrid>>(cachedSetting);
            else
            {
                lockoutSetting = await _security.ScrewIdentifierGrid.Where(a => a.Module == (int)Modules.CENTRAL).ToListAsync();
                await _cacheService.CatcheResponseAsync(CacheKeys.central_authSettings, lockoutSetting);
            }
            return lockoutSetting;
        }
        ...
        public async Task<LogingFailedRespObj> CheckForFailedTrailsAsync(bool isSuccessful, int module, string userid)
        {
            ...
            if(lockoutSetting.Count() > 0 )await _cacheService.CatcheResponseAsync(CacheKeys.AuthSettings, lockoutSetting); 
