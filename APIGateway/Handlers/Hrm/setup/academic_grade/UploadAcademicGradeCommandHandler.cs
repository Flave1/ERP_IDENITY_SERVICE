...
{
    public class UploadAcademicGradeCommand : IRequest<FileUploadRespObj>
    {
        public class UploadAcademicGradeCommandHandler : IRequestHandler<UploadAcademicGradeCommand, FileUploadRespObj>
        {
            private readonly IHttpContextAccessor _accessor;
            private readonly DataContext _context;
            private readonly ISetupRepository _setup;
            public UploadAcademicGradeCommandHandler(ISetupRepository setup, IHttpContextAccessor accessor, DataContext context)
            {
                _accessor = accessor;
                _context = context;
                _setup = setup;
            ...
            public async Task<FileUploadRespObj> Handle(UploadAcademicGradeCommand request, CancellationToken cancellationToken)
            {
                var response = new FileUploadRespObj { Status = new APIResponseStatus { IsSuccessful = false, Message = new APIResponseMessage() } };


                List<hrm_setup_academic_grade_contract> uploadedRecord = new List<hrm_setup_academic_grade_contract>();
                    ...
                    if (byteList.Count() > 0)
                    {
                        foreach (var item in byteList)
                        {
                            ...
                            using (ExcelPackage excelPackage = new ExcelPackage(stream))
                            {
                                ...
                                {
                                    uploadedRecord.Add(new hrm_setup_academic_grade_contract
                                    {
                                        ExcelLineNumber = i,
                ...
                try
                {
                    if (uploadedRecord.Count > 0)
                    {
                        foreach (var item in uploadedRecord)
                        {
                            ...
                            var current_item = _context.hrm_setup_academic_grade.FirstOrDefault(e => e.Grade.ToLower() == item.Grade.ToLower());
                            
                            if(current_item == null)
                            {
                                current_item = new hrm_setup_academic_grade();
                                current_item.Grade = item.Grade;
                                current_item.Description = item.Description;
                                current_item.Rank = item.Rank;
                                await _setup.AddUpdateAcademicGradeAsync(current_item);
                            }
                            else
                            {
                                current_item.Grade = item.Grade;
