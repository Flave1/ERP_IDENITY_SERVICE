...
{
    public class Deletehrm_setup_jobtitle : IRequest<DeleteRespObj>
    {
        public List<int> ItemIds { get; set; }
        public class Deletehrm_setup_jobtitleHandler : IRequestHandler<Deletehrm_setup_jobtitle, DeleteRespObj>
        {
            private readonly ILoggerService _logger;
            private readonly DataContext _data;
            public Deletehrm_setup_jobtitleHandler(
            ...
            public async Task<DeleteRespObj> Handle(Deletehrm_setup_jobtitle request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    if (request.ItemIds.Count > 0)
                    {
                        ...
                        {
                            var selected = await _data.hrm_setup_jobtitle.FindAsync(Id);
                            if (selected != null) 
                                _data.hrm_setup_jobtitle.Remove(selected); 
                        }
