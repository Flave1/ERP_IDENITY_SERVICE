...
{
    public class Deletehrm_setup_employmentlevel : IRequest<DeleteRespObj>
    {
        public List<int> ItemIds { get; set; }
        public class Deletehrm_setup_employmentlevelHandler : IRequestHandler<Deletehrm_setup_employmentlevel, DeleteRespObj>
        {
            private readonly ILoggerService _logger;
            private readonly DataContext _data;
            public Deletehrm_setup_employmentlevelHandler(
            ...
            public async Task<DeleteRespObj> Handle(Deletehrm_setup_employmentlevel request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    if (request.ItemIds.Count > 0)
                    {
                        ...
                        {
                            var selected = await _data.hrm_setup_employmentlevel.FindAsync(Id);
                            if (selected != null) 
                                _data.hrm_setup_employmentlevel.Remove(selected); 
                        }
