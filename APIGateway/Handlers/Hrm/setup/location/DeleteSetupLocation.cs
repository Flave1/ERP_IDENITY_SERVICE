...
{
    public class Deletehrm_setup_location : IRequest<DeleteRespObj>
    {
        public List<int> ItemIds { get; set; }
        public class Deletehrm_setup_locationHandler : IRequestHandler<Deletehrm_setup_location, DeleteRespObj>
        {
            private readonly ILoggerService _logger;
            private readonly DataContext _data;
            public Deletehrm_setup_locationHandler(
            ...
            public async Task<DeleteRespObj> Handle(Deletehrm_setup_location request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    if (request.ItemIds.Count > 0)
                    {
                        ...
                        {
                            var selected = await _data.hrm_setup_location.FindAsync(Id);
                            if (selected != null)
                                _data.hrm_setup_location.Remove(selected);
                        }
