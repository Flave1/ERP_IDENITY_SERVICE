...
{
    public class Deletehrm_setup_employmenttype : IRequest<DeleteRespObj>
    {
        public List<int> ItemIds { get; set; }
        public class Deletehrm_setup_employmenttypeHandler : IRequestHandler<Deletehrm_setup_employmenttype, DeleteRespObj>
        {
            private readonly ILoggerService _logger;
            private readonly DataContext _data;
            public Deletehrm_setup_employmenttypeHandler(
            ...
            public async Task<DeleteRespObj> Handle(Deletehrm_setup_employmenttype request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    if (request.ItemIds.Count > 0)
                    {
                        ...
                        {
                            var selected = await _data.hrm_setup_employmenttype.FindAsync(Id);
                            if (selected != null) 
                                _data.hrm_setup_employmenttype.Remove(selected); 
                        }
