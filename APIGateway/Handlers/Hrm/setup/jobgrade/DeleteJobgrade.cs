...
{
    public class Deletehrm_setup_jobgrade : IRequest<DeleteRespObj>
    {
        public List<int> ItemIds { get; set; }
        public class Deletehrm_setup_jobgradeHandler : IRequestHandler<Deletehrm_setup_jobgrade, DeleteRespObj>
        {
            private readonly ILoggerService _logger;
            private readonly DataContext _data;
            public Deletehrm_setup_jobgradeHandler(
            ...
            public async Task<DeleteRespObj> Handle(Deletehrm_setup_jobgrade request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    if (request.ItemIds.Count > 0)
                    {
                        ...
                        {
                            var selected = await _data.hrm_setup_jobgrade.FindAsync(Id);
                            if (selected != null)
                                _data.hrm_setup_jobgrade.Remove(selected);
                        }
