...
{
    public class Deletehrm_setup_hmo : IRequest<DeleteRespObj>
    {
        public List<int> ItemIds { get; set; }
        public class Deletehrm_setup_hmoHandler : IRequestHandler<Deletehrm_setup_hmo, DeleteRespObj>
        {
            private readonly ILoggerService _logger;
            private readonly DataContext _data;
            public Deletehrm_setup_hmoHandler( ILoggerService loggerService, DataContext data)
            {
                _logger = loggerService;
                _data = data;
            }
            public async Task<DeleteRespObj> Handle(Deletehrm_setup_hmo request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    if (request.ItemIds.Count > 0)
                    {
                        ...
                        {
                            var selected = await _data.hrm_setup_hmo.FindAsync(Id);
                            if (selected != null) 
                                _data.hrm_setup_hmo.Remove(selected); 
                        }
