...
{
    public class DeleteEmp_CareerCommand : IRequest<DeleteRespObj>
    {
        public List<int> ItemIds { get; set; }
        public class DeleteEmp_CareerCommandHandler : IRequestHandler<DeleteEmp_CareerCommand, DeleteRespObj>
        {
            private readonly ILoggerService _logger;
            private readonly DataContext _data;
            private readonly IEmployeeRepository _empRepo;
            public DeleteEmp_CareerCommandHandler(ILoggerService loggerService, DataContext data, IEmployeeRepository empRepo)
            {
            ...
            public async Task<DeleteRespObj> Handle(DeleteEmp_CareerCommand request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    if (request.ItemIds.Count() > 0)
                    {
                        ...
                        {
                            await _empRepo.DeleteEmpCareerAsync(itemId);
                        }
                    }
