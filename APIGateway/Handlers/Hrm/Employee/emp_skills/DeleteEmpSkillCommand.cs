...
{
    public class DeleteEmp_skillCommand : IRequest<DeleteRespObj>
    {
        public List<int> ItemIds { get; set; }
        public class DeleteEmp_skillCommandHandler : IRequestHandler<DeleteEmp_skillCommand, DeleteRespObj>
        {
            private readonly ILoggerService _logger;
            private readonly DataContext _data;
            private readonly IEmployeeRepository _empRepo;
            public DeleteEmp_skillCommandHandler(ILoggerService loggerService, DataContext data, IEmployeeRepository empRepo)
            {
            ...
            public async Task<DeleteRespObj> Handle(DeleteEmp_skillCommand request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    if (request.ItemIds.Count() > 0)
                    {
                        ...
                        {
                            await _empRepo.DeleteEmpSkillAsync(itemId);
                        }
                    }
