...
{
    public class DeleteEmp_AssetCommand : IRequest<DeleteRespObj>
    {
        public List<int> ItemIds { get; set; }
        public class DeleteEmp_AssetCommandHandler : IRequestHandler<DeleteEmp_AssetCommand, DeleteRespObj>
        {
            private readonly ILoggerService _logger;
            private readonly DataContext _data;
            private readonly IEmployeeRepository _empRepo;
            public DeleteEmp_AssetCommandHandler(ILoggerService loggerService, DataContext data, IEmployeeRepository empRepo)
            {
            ...
            public async Task<DeleteRespObj> Handle(DeleteEmp_AssetCommand request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    if (request.ItemIds.Count() > 0)
                    {
                        ...
                        {
                            await _empRepo.DeleteEmpAssetAsync(itemId);
                        }
                    }
