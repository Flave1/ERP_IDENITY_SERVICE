...
{
    public class DeleteEmp_HobbiesCommand : IRequest<DeleteRespObj>
    {
        public List<int> ItemIds { get; set; }
        public class DeleteEmp_HobbiesCommandHandler : IRequestHandler<DeleteEmp_HobbiesCommand, DeleteRespObj>
        {
            private readonly ILoggerService _logger;
            private readonly DataContext _data;
            private readonly IEmployeeRepository _empRepo;
            public DeleteEmp_HobbiesCommandHandler(ILoggerService loggerService, DataContext data, IEmployeeRepository empRepo)
            {
            ...
            public async Task<DeleteRespObj> Handle(DeleteEmp_HobbiesCommand request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    if (request.ItemIds.Count() > 0)
                    {
                        ...
                        {
                            await _empRepo.DeleteEmpHobbyAsync(itemId);
                        }
                    }
