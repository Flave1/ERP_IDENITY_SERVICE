...
{
    public class DeleteEmp_RefereeCommand : IRequest<DeleteRespObj>
    {
        public List<int> ItemIds { get; set; }
        public class DeleteEmp_RefereeCommandHandler : IRequestHandler<DeleteEmp_RefereeCommand, DeleteRespObj>
        {
            private readonly ILoggerService _logger;
            private readonly DataContext _data;
            private readonly IEmployeeRepository _empRepo;
            public DeleteEmp_RefereeCommandHandler(ILoggerService loggerService, DataContext data, IEmployeeRepository empRepo)
            {
            ...
            public async Task<DeleteRespObj> Handle(DeleteEmp_RefereeCommand request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    if (request.ItemIds.Count() > 0)
                    {
                        ...
                        {
                            await _empRepo.DeleteEmpRefereeAsync(itemId);
                        }
                    }
