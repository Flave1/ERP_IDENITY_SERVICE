            ...
            public async Task<CompanyExchangeRateRespObj> Handle(GetExchangeRateQuery request, CancellationToken cancellationToken)
            {
                ...
                if (baseCompany.CurrencyId > 0)
                {
                    ...
                    else
                    {
                       ...
                       // apiResponse.BaseCurrencyId = baseCurrency.CurrencyId;
                        apiResponse.CurrencyId = request.CurrencyId;
                        apiResponse.BuyingRate = Convert.ToDouble(rateInfo.BuyingRate);
                        apiResponse.SellingRate = Convert.ToDouble(rateInfo.SellingRate);
                        apiResponse.Date = rateInfo.Date;
                        apiResponse.IsBaseCurrency = false;
                        apiResponse.Status.IsSuccessful = true;
                        return apiResponse;
                    }
                }
                ...
                {
                    apiResponse.Status.Message.FriendlyMessage = "This company does not have currency";
                    apiResponse.Status.IsSuccessful = true;
                    return apiResponse;
                }
            }
