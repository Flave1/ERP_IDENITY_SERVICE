...
namespace APIGateway.Handlers.Workflow
{
    public class GoForApprovalCommandHandler : IRequestHandler<GoForApprovalCommand, GoForApprovalRespObj>
    {
                    ...
                    try
                    {
                        ...
                        { 
                            ...
                            foreach (var staff in WorkflowLevelStaffObject)
                            {
                                ...
                                appproverTask.Directory_link = request.Directory_link;
                                if (request.WorkflowTaskId < 1)
                                {
                                    ...
                                    if (appproverTask.Position == 1)
                                    {
                                        ...
                                        await SendMail(appproverTask, staff.Name);
                                        //Send Mail...................
                                    }
                                    if (!await _repo.CreateUpdateWorkflowTaskAsync(appproverTask))
                                    {
                                        ...
                                        return new GoForApprovalRespObj
                                        {
                                            ...
                                            HasWorkflowAccess = true,
                                            ApprovalProcessStarted = false,
                                            Status = new APIResponseStatus { IsSuccessful = true, Message = new APIResponseMessage { FriendlyMessage = "Approval Process failed to start" } }
                                        };
                                    }
                                }
                            }
        ...
        private async Task SendMail(cor_workflowtask workflowtask, string username)
        {
            var button = _repo.Return_workflow_button_link(workflowtask.Directory_link);
            var put_button = !string.IsNullOrEmpty(workflowtask.Directory_link) ? button : string.Empty;
            var sm = new SendEmailCommand();
            sm.Subject = $"{workflowtask.Comment} awaiting approval";
            sm.Content = $"Hello {username} <br/>" +
                $"You have a pending task on {workflowtask.Comment} as the first approver " + 
                $"<br/> <br/>{put_button}";
            sm.ToAddresses.Add(new EmailAddressCommand { Address = workflowtask.StaffEmail, Name = username });
            sm.SendIt = true;
            sm.SaveIt = true;
