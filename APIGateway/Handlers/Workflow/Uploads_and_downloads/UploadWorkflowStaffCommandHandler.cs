                    ...
                    if (byteList.Count() > 0)
                    {
                        foreach (var byteItem in byteList)
                        {
                            ...
                            using (ExcelPackage excelPackage = new ExcelPackage(stream))
                            {
                                ...
                                if (totalColumns != 4)
                                {
                                    apiResponse.Status.Message.FriendlyMessage = $"Four (4) Column Expected";
                                    return apiResponse;
                                }
                                ...
                                for (int i = 2; i <= totalRows; i++)
                                {
                                    var lkp = new WorkflowlevelStaffObj
                                    {
                                        ...
                                        AccessLevel = workSheet.Cells[i, 3]?.Value != null ? workSheet.Cells[i, 3]?.Value.ToString() : string.Empty,
                                        StaffCode = workSheet.Cells[i, 4]?.Value != null ? workSheet.Cells[i, 4]?.Value.ToString() : string.Empty,
                                    };
                    ...
                    if (uploadedRecord.Count > 0)
                    {
                        foreach (var row in uploadedRecord)
                        {
                            ...
                            else
                            {
                                ...
                                var level = _StaffList.FirstOrDefault(a => a.StaffCode.Trim().ToLower() == row.StaffCode.Trim().ToLower());
                                if (level == null)
                                {
