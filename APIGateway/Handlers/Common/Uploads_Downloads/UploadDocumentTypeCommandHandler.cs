        ...
        public class UploadDocumentTypeCommandHandler : IRequestHandler<UploadDocumentTypeCommand, FileUploadRespObj>
        {
            ...
            public async Task<FileUploadRespObj> Handle(UploadDocumentTypeCommand request, CancellationToken cancellationToken)
            {
                try
                {
                            ...
                            using (ExcelPackage excelPackage = new ExcelPackage(stream))
                            {
                                ...
                                for (int i = 2; i <= totalRows; i++)
                                {
                                    ...
                                    { 
                                        ...
                                        ParentName = workSheet.Cells[i, 2]?.Value != null ? workSheet.Cells[i, 2]?.Value.ToString() : string.Empty,
                                    };
                    ...
                    {
                        if(uploadedRecord.Count(e => e.ParentName == Doc_identifier.Signature.ToString()) > 1)
                        {
                            apiResponse.Status.Message.FriendlyMessage = $"Multiple signatures detected";
                            return apiResponse;
                        }
                        ...
                        foreach (var item in uploadedRecord)
                        {
                            ...
                            {
                                apiResponse.Status.Message.FriendlyMessage = $"Document name can not be empty detected on line {item.ExcelLineNumber}";
                                return apiResponse;
                            }
                            if (string.IsNullOrEmpty(item.ParentName))
                            {
                                apiResponse.Status.Message.FriendlyMessage = $"Type name can not be empty detected on line {item.ExcelLineNumber}";
                                return apiResponse;
                            }

                            if (item.ParentName.ToLower() != "signature" && item.ParentName.ToLower() != "other_document")
                            {
                                apiResponse.Status.Message.FriendlyMessage = $"Unidetified Type name detected on line {item.ExcelLineNumber}";
                                return apiResponse;
                            }
                            ...
                            if(currentDocumentType == null)
                            {
                                ...
                                DocumentType.Doc_identifier = item.ParentName.ToLower() == "signature" ? 1 : 2;
                                await _repo.AddUpdateDocumentTypeAsync(DocumentType);
                            }
                            else
                            {
                                currentDocumentType.Name = item.LookupName;
                                currentDocumentType.Doc_identifier = item.ParentName.ToLower() == "signature" ? 1 : 2;
