        ...
        public async Task<FileUploadRespObj> Handle(UploadCompanyStructureCommand request, CancellationToken cancellationToken)
        {
                ...
                if(byteList.Count() > 0)
                {
                    foreach (var item in byteList)
                    {
                        ...
                        using (ExcelPackage excelPackage = new ExcelPackage(stream))
                        {
                            ...
                            if (totalColumns != 5)
                            {
                                response.Status.Message.FriendlyMessage = $"Five (5) Column Expected";
                                return response;
                            }
                            for (int i = 2; i <= totalRows; i++)
                            {
                                uploadedRecord.Add(new CompanyStructureObj
                                {
                                    ...
                                    StaffCode = workSheet.Cells[i, 3].Value != null ? workSheet.Cells[i, 3].Value.ToString() : null,
            ...
            try
            {
                if (uploadedRecord.Count > 0)
                {
                    ...
                    if (uploadedRecord.Count(e => string.IsNullOrEmpty(e.ParentCompanyName)) != 1)
                    {
                        response.Status.Message.FriendlyMessage = $"Please confirm the setup for a group company";
                        return response;
                    }

                    if (!uploadedRecord.Any(e => string.IsNullOrEmpty(e.ParentCompanyName)))
                    {
                        response.Status.Message.FriendlyMessage = $"Please confirm the setup for a group company";
                        return response;
                    }
                    ...
                    foreach (var item in uploadedRecord)
                    {
                        ...
                        if (item.StructureTypeName != null)
                        {
                            ...
                            var group_level = _CompanyStructureDefinition.Select(e => e.StructureLevel).OrderBy(e => e).ToList().FirstOrDefault();
                            if (data.StructureLevel == group_level)
                            {
                                item.ParentCompanyID = 0;
                            }
                        }
                        else
                        {
                        ...
                        if (string.IsNullOrEmpty(item.StaffCode))
                        {
                            response.Status.Message.FriendlyMessage = $"Staff Code can not be empty detected on line {item.ExcelLineNumber}";
                            return response;
                        }
                        else
                        { 
                            var staff = _StaffList.FirstOrDefault(a => a.StaffCode.Trim().ToLower() == item.StaffCode.Trim().ToLower());
                            if (staff == null)
                            {
                                response.Status.Message.FriendlyMessage = $"Unidentified Staff code Detected on line {item.ExcelLineNumber}";
                                return response;
                            }
                        ...
                        else
                        {
                            ...
                            if (!string.IsNullOrEmpty(item.ParentCompanyName))  parentname = item.ParentCompanyName; 
                            var title = _CompanyStructure.FirstOrDefault(x => x.Name.ToLower().Contains(parentname.ToLower()));
                            if (title == null)
                            {
